...
We can refer to Figure 3 as [Figure 3](#figure-Figure3).
...
{% include image.html
  name="Figure 1"
  ref="Figure1"
  alt="Protein generated by RFDiffusion."
...
Flow Matching (FM) models are in nature most closely related to (Continuous) Normalising Flows (CFM).
Therefore, we start this blogpost by briefly recapping the core concepts behind Normalising Flows. We then continue by 


## Basics: Normalising 

Let $\phi: \mathbb{R}^d \rightarrow \mathbb{R}^d$ be a continuously differentiable function which transforms or maps elements of $\mathbb{R}^d$, with a continously differentiable inverse $\phi^{-1}: \mathbb{R}^d \to \mathbb{R}^d$.
...
$$
q \big( \phi(x) \big) = \frac{p(x)}{\abs{\det\left[\frac{\partial \phi}{\partial x}(x)\right]}}
$$

which can be seen from the fact that $\phi \circ \phi^{-1} = \Id$ and a simple application of the chain rule[^chainrule].

This formulation is very useful during sampling as we can compute both $y = \phi(x)$ and $q(y) \propto \abs{\det\left[\frac{\partial \phi}{\partial x}(x)\right]}^{-1}$ in the same forward-pass of our model.
...
<div class="my-image-container">
  ...
  <!-- alt=. -->
{% include image.html
  name="Figure: $\phi_t(x_0)$ for a few samples $x_0 \sim p_0 = \mathcal{N}(0, 1)$ coloured according to $p_0(x_0)$"
...
{% include image.html
  name="Figure: Jacobian structure of different normalising flows."
...
{% include image.html
  name="Figure: Probability path $p_t = \mathcal{N}(\mu t, 1)$ from $p_0 = \mathcal{N}(0, 1)$ to $p_1 = \mathcal{N}(\mu, 1)$"
...
{% include image.html
  ref="forward_samples-one-color-1.png"
...
{% include image.html
  name="Figure: Two probability path $p_t(x|x_1)$"
  ref="heatmap_with_cond_traj-v3.png"
...
\end{equation}
$$
for some $\mu > 0$. We're effectively transforming a Gaussian to another Gaussian using a simple time-linear map, as illustratede in the following figure.
...
Figure: *Marginal vector field $u_t(x)$ vs. conditional vector field $u_t(x \mid x_1)$ for samples $x_1 \sim p_1$. Here $p_0 = p_1 = \mathcal{N}(0, 1)$ and the two trajectories are according to the marginal vector field $u_t(x)$. Samples $x_1$ transparency is given by the IS weight $p_t(x \mid x_1) / p_t(x)$.*
...
{% include image.html
  name="Figure: Realizations of paths from $p_0$ to $p_1$ following conditional vector fields $u_t(x \mid x_1)$. Paths are highlighted by the sign of the 2nd vector component at time $t=1$"
...
{% include image.html
  name="Figure: Paths from $p_0$ to $p_1$ following the true marginal vector field $u_t(x)$. Paths are highlighted by the sign of the 2nd vector component"
...
<!-- 1. We ideally want paths to be straight, because straight paths are easy to numerically integrate, i.e. inference is fast.
2. Conditional vector fields do not prohibit "crossing paths", while the marginal vector fields do prohibit this. -->
...
{% include image.html
  name="Figure: Realizations of conditional paths from $p_0 = p_1 = \mathcal{N}(0, 1)$ for two different $x_1^{(i)}, x_1^{(2)} \sim q$ with conditional vector field given by $u_t(x \mid x_1) = (1 - t) x + t x_1$"
  ref=".png"
  src="flow-matching/g2g-forward_samples.png"
  width=400
%}
...
{% include image.html
  name="Figure: Realizations of conditional paths from $p_0 = \mathcal{N}([-\mu, 0], I)$ to $p_1 = \mathcal{N}([\mu, 0], I)$ following realizations of the conditional vector field $u_t(x \mid x_1)$. Paths are highlighted by the sign of the 2nd vector component"
...
{% include image.html
  name="Figure: Paths from $p_0 = \mathcal{N}([-\mu, 0], I)$ to $p_1 = \mathcal{N}([\mu, 0], I)$ following the true marginal vector field $u_t(x)$. Paths are highlighted by the sign of the 2nd vector component"
...
{% include image.html
  name="Figure: Realizations of conditional paths from $p_0 = \mathcal{N}([-\mu, 0], I)$ to $p_1 = \mathcal{N}([\mu, 0], I)$ following realizations of the conditional vector field $u_t(x \mid x_1)$"
...
{% include image.html
  name="Figure: Variance of conditional vector field over $p_{1|t}$ for both blue and red trajectories"
...
Similarly to CNFs, sampled can be obtained at inference time by solving the ODE associated with the neural drit.
...
- Albergo, Boffi & Vanden-Eijnden (2023) Stochastic Interpolants: a Unifying Framework for Flows and Diffusions. URL: http://arxiv.org/abs/2303.08797v3.

- Chen & Lipman (2023) Riemannian Flow Matching on General Geometries. URL: http://arxiv.org/abs/2302.03660v2

- De Bortoli, Mathieu & Hutchinson et al. (2022) Riemannian Score-Based Generative Modelling. URL: http://arxiv.org/abs/2202.02763v3.

- Dupont, Doucet & Teh (2019) Augmented Neural Odes. URL: http://arxiv.org/abs/1904.01681v3.

- Klein, Krämer & Noé (2023) Equivariant Flow Matching. URL: http://arxiv.org/abs/2306.15030v2.

- Lipman, Chen & Ben-Hamu et al. (2022) Flow Matching for Generative Modeling. URL: http://arxiv.org/abs/2210.02747v2.

- Song, Sohl-Dickstein & Kingma et al. (2020) Score-Based Generative Modeling Through Stochastic Differential Equations. URL: http://arxiv.org/abs/2011.13456v2.

- Tong, Malkin & Huguet et al. (2023) Improving and Generalizing Flow-Based Generative Models With Minibatch Optimal Transport. URL: http://arxiv.org/abs/2302.00482v2.
